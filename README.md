...
- `LINKEDIN_API_VERSION` — the supported LinkedIn REST API version in `YYYYMM` format
- `LINKEDIN_ORGANIZATION_ID` — the numeric ID of the openings.dev LinkedIn Page
...
- `LINKEDIN_ACCESS_TOKEN` — requires the LinkedIn `w_organization_social` and `r_organization_social` permissions for the configured Page
...
For LinkedIn, run `controlled` with the `publish_linkedin` input enabled while `LINKEDIN_AUTO_PUBLISH` remains false. Keep a flag disabled if its token, public media, or durable result cannot be verified. Long-lived Meta tokens and the LinkedIn access token must be renewed before expiry. The Instagram account must be a professional Business account authorized for content publishing.
