...
Bluesky receives an explicit external card. LinkedIn receives an article post with the canonical job URL and the existing 1200×630 image as its explicit thumbnail. Mastodon and Threads receive the canonical link so the public Open Graph preview can resolve naturally. Instagram receives a verified nine-second Reel with a dedicated cover, production caption, and an original soundtrack generated by this repository. The same H.264/AAC file is ready for a later YouTube Shorts rollout. X is not included because its write API is not part of the free rollout.
...
- Instagram, Threads, and LinkedIn are independent opt-ins. Jobs already known when a channel is activated are never backfilled and keep LinkedIn as `skipped_before_activation`.
...
The production sequence for a new channel is credentials → controlled post → duplicate-reconciliation check → channel flag. For LinkedIn, run `controlled` with the `publish_linkedin` input enabled while `LINKEDIN_AUTO_PUBLISH` remains false. Keep that channel disabled if its token, public media, or durable result cannot be verified. Long-lived Meta tokens and the LinkedIn access token must be renewed before expiry.
