...
export function validateFirebaseServiceAccount(rawJson) {
  ...
  try {
    const keyObject = createPrivateKey(credential.private_key);
...
export async function configureOneSignalFcm({
  ...
  if (/[\r\n]/u.test(organizationApiKey)) {
    throw new Error('OneSignal Organization API key is invalid');
  }
  ...
  let updateResponse;
  try {
    updateResponse = await fetchImpl(endpoint, {
      method: 'PUT',
      headers: {
    ...
    throw new Error('OneSignal FCM update transport failed');
  }

  if (typeof updateResponse?.ok !== 'boolean') {
    throw new Error('OneSignal FCM update transport failed');
  }
  ...
  try {
    readBackResponse = await fetchImpl(endpoint, {
      method: 'GET',
      headers: {
        accept: 'application/json',
        authorization,
    ...
    throw new Error('OneSignal FCM read-back transport failed');
  }

  if (typeof readBackResponse?.ok !== 'boolean') {
    throw new Error('OneSignal FCM read-back transport failed');
  }
  ...
  let app;
  try {
    app = await readBackResponse.json();
  } catch {
    throw new Error('OneSignal FCM read-back response is invalid');
  }
