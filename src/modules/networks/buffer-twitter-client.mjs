...
const TWITTER_HOST_PATTERN = /^(?:[a-z0-9-]+\.)*(?:twitter|x)\.com$/u;
...
function normalizeConfiguration({
  ...
  return Object.freeze({
    ...
    channelId: requiredIdentifier(channelId, 'Twitter channel ID'),
...
async function loadTwitterChannel(configuration) {
  ...
  if (!channel
    ...
    throw publicationError('buffer_configuration', 'Buffer Twitter channel is unavailable');
  }
  return Object.freeze({ id: channel.id, service: channel.service });
}

export async function verifyBufferTwitterChannel({
  apiKey,
  organizationId,
  channelId,
  ...
  return loadTwitterChannel(configuration);
}
...
function normalizeTwitterUrl(value) {
  ...
  try {
    ...
    throw publicationError('buffer_response', 'Buffer returned an invalid Twitter URL');
  }
  if (url.protocol !== 'https:'
    || url.username
    || url.password
    || !TWITTER_HOST_PATTERN.test(url.hostname.toLowerCase())) {
    throw publicationError('buffer_response', 'Buffer returned an invalid Twitter URL');
  }
  return url.toString();
}
...
async function createPost(configuration, publication, imageUrl) {
  ...
  return normalizePost(data.createPost.post, 'CreateTwitterPost');
}
...
function completedResult(post, status = 'published') {
  ...
  return Object.freeze({
    status,
    id: post.id,
    url: normalizeTwitterUrl(post.externalLink),
    provider: 'buffer',
  });
}
...
async function pollPost(configuration, postId, polling, resultStatus = 'published') {
  for (let attempt = 0; attempt < polling.pollAttempts; attempt += 1) {
    ...
    if (post.status === 'error') {
      throw publicationError('buffer_publication', 'Buffer could not publish the Twitter post');
    }
...
export async function publishToTwitterViaBuffer({
  job,
  post,
  imageUrl,
  publicSiteOrigin,
  ...
  await loadTwitterChannel(configuration);
