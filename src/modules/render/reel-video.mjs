...
function stageSvg({ model, wordmark }, stage) {
  ...
  <g data-reel-brand="true" opacity="1" data-important-content="true" data-x="${geometry.header.x}" data-y="${geometry.header.y}" data-width="${geometry.header.width}" data-height="${geometry.header.height}">
    <image x="180" y="338" width="220" height="40" preserveAspectRatio="xMinYMid meet" href="${wordmark}"/>
    <text x="900" y="350" text-anchor="end" fill="${colors.ink}" font-family="${SOCIAL_CARD_FONT_STACK}" font-size="21" font-weight="850">${escapeHtml(model.handle)}</text>
    <text x="900" y="385" text-anchor="end" fill="${colors.mutedInk}" font-family="${SOCIAL_CARD_FONT_STACK}" font-size="16" font-weight="750">Tech jobs from public communities</text>
  </g>
  <g data-reel-title="true" opacity="${titleOpacity}" data-important-content="true" data-x="${geometry.role.x}" data-y="${geometry.role.y}" data-width="${geometry.role.width}" data-height="${geometry.role.height}">
    ${textLines(eyebrowLines, { x: 180, y: 490, fontSize: 18, lineHeight: 24, fill: colors.mutedInk, weight: 850, attribute: 'letter-spacing="1.7"' })}
    ${textLines(layout.titleLines, { x: 180, y: Math.round(titleY), fontSize: layout.titleFontSize, lineHeight: layout.titleLineHeight, fill: colors.ink, weight: 900, attribute: 'letter-spacing="-2.8" data-reel-title-line="true"' })}
  </g>
  ...
  <g data-reel-attribution="true" opacity="${attributionOpacity}" data-important-content="true" data-x="${geometry.attribution.x}" data-y="${geometry.attribution.y}" data-width="${geometry.attribution.width}" data-height="${geometry.attribution.height}">
    <text x="180" y="1550" fill="${colors.ink}" font-family="${SOCIAL_CARD_FONT_STACK}" font-size="14" font-weight="850" letter-spacing="1.6">${escapeHtml(model.attribution.label)}</text>
    ${textLines(attributionLines, { x: 180, y: 1600, fontSize: 28, lineHeight: 34, fill: colors.ink, weight: 850 })}
    <rect x="520" y="1540" width="380" height="104" rx="52" fill="${colors.ink}"/>
    <text x="545" y="1602" fill="${colors.paper}" font-family="${SOCIAL_CARD_FONT_STACK}" font-size="16" font-weight="800">${escapeHtml(model.attribution.action)}</text>
    <text x="875" y="1608" text-anchor="end" fill="${model.theme.accent}" font-family="${SOCIAL_CARD_FONT_STACK}" font-size="30" font-weight="850">→</text>
  </g>
</svg>`;
}
