...
import { migrateTwitterState } from '../modules/state/twitter-state-migration.mjs';
...
export async function runTwitterStateMigration({
  stateDirectory,
  at = new Date().toISOString(),
  confirmation,
}) {
  if (confirmation !== 'MIGRATE_TWITTER_STATE') {
    throw new Error('Twitter state migration requires the exact confirmation phrase');
  }
  const queuePath = resolve(stateDirectory, 'queue.json');
  const publicationsPath = resolve(stateDirectory, 'publications.json');
  const [queueState, publicationsState] = await Promise.all([
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  main().catch((error) => {
    console.error(error instanceof Error ? error.message : 'Twitter state migration failed');
    process.exitCode = 1;
  });
}
