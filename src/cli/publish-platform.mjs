...
import { prepareSocialPublication, submitSocialPublication, readSocialPublication } from '../modules/publishing/platform-publisher.mjs';
...
export async function runPlatformCommand(args, env = process.env) {
  ...
  for (let index = 0; index < values.length; index += 2) {
    ...
    if (!['--job', '--media', '--outbox', '--handoff', '--publication'].includes(name)
  ...
  if (command === 'prepare' && options['--job'] && options['--media'] && !options['--handoff'] && !options['--publication']) {
  ...
  if (command === 'status' && options['--publication'] && Object.keys(options).length === 1) {
    return readSocialPublication({ publicationId: options['--publication'], transport: {
  ...
  throw new Error('Use prepare --job FILE --media PNG, submit --handoff FILE, or status --publication ID');
}
